...
| ⚙️ `context-guard` | Automático (PostToolUse) — setup via `/context-guard:setup` | Auto-interrompe o workflow quando o context window ultrapassa threshold configurável (default: 80%). Pensado pra modelos com contexto limitado — o threshold de 80% assume modelos como Opus 4.6 (200k tokens). Quando dispara, sugere `/handoff`. **Requer `handoff` instalado.** |
| `handoff` | `/handoff` (salvar) · `/continue` (carregar) | Ciclo completo de continuidade: cria `.claude/HANDOFF.md` no fim da sessão e carrega de volta na próxima pra retomar exatamente de onde parou. |
