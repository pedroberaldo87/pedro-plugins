...
| --- | --- | --- |
| hooks de produção — o que algum `hooks/hooks.json` registra, resolvido pelo medidor oficial | **41** | `python3 scripts/hook_contract.py --scripts \| grep -c .` |
| os `.sh` de hooks que leem o campo que DECIDE — classe B | **36** | ver classe B abaixo — três deles pela forma do Python embutido; a conta inclui os chamados por outro hook, sem registro próprio |
...
O aviso de dependência é o caso mais visível: são os plugins que
`grep -l 'sessionstart-deps.sh' plugins/*/hooks/hooks.json | wc -l` conta que o disparam na
abertura da sessão, porque quem instala um plugin sozinho também precisa saber que o gate dele
ficou mudo — mas o script existe UMA vez só, em `plugins/bootstrap/hooks/sessionstart-deps.sh`,
e os demais o acham por NOME de plugin (`resolve-plugin.sh bootstrap hooks/sessionstart-deps.sh`) em vez
...
Hoje os 33 da forma comum leem pelo `_shared/hook-json.sh`, vendorado em cada pasta de
...
| --- | --- | --- | --- |
| A | `plugins/bootstrap/hooks/session-sync.sh` | 181, 182 | interroga `known_marketplaces.json` (arquivo de config, não o payload do evento) |
| A | `plugins/bootstrap/hooks/sessionstart-deps.sh` | 19 | só confere se o `jq` existe para avisar da falta — a mensagem sai sem ele |
| A | `plugins/branches/hooks/sessionstart-branches.sh` | 12, 15, 21, 34, 44 | lê `.cwd` e serializa o `additionalContext` |
| A | `plugins/graphify-guard/hooks/sessionstart-graphify.sh` | 8, 11, 36, 38 | lê `.cwd` e serializa o `additionalContext` |
...
| B1 | `plugins/graphify-guard/hooks/pretooluse-graphify-guard.sh` | 36, 50 | `session_id`, `tool_input.command` | `permissionDecision:"deny"` |
| B1 | `plugins/guardrails/hooks/askq-humanize.sh` | 48 | `session_id` | `permissionDecision:"deny"` |
| B1 | `plugins/guardrails/hooks/lint-and-typecheck.sh` | 33 | `session_id` | `exit 2` |
| B1 | `plugins/guardrails/hooks/scope-cop.sh` | 79 | `session_id` | `permissionDecision: "deny"` |
| B1 | `plugins/intent-guard/hooks/delivery-audit.sh` | 31 | `session_id` | `decision:"block"` |
| B1 | `plugins/intent-guard/hooks/plan-gate.sh` | 30 | `session_id` | `exit 2` — sem registro próprio: chamado pelo portão único da família |
| B1 | `plugins/intent-guard/hooks/task-checkpoint.sh` | 30 | `session_id` | `decision:"block"` |
| B1 | `plugins/project-skills/hooks/pretooluse-doc-guard.sh` | 41, 54 | `session_id`, `tool_input.command` | `permissionDecision:"deny"` |
| B1 | `plugins/project-skills/hooks/pretooluse-organism-gate.sh` | 49 | `session_id` | `permissionDecision:"deny"` |
| B1 | `plugins/project-skills/hooks/pretooluse-plan-gate.sh` | 69 | `session_id` | `permissionDecision:"deny"` |
| B1 | `plugins/ship/hooks/pre-deploy-test-check.sh` | 30 | `tool_input.command` | `exit 2` |
...
| B1 | `plugins/project-skills/hooks/pretooluse-motor-arma.sh` | 64 | `session_id` | `permissionDecision:"deny"` |
| B1 | `plugins/visual/hooks/pre-exitplan-visualize.sh` | 32 | `session_id` | `exit 2` — sem registro próprio: chamado pelo portão único da família |
...
| --- | --- | --- | --- | --- |
| B2 | `plugins/bootstrap/hooks/post-plugin-command.sh` | 59 | `tool_input.command` | mutação de plugin não é detectada; o snapshot/commit nunca roda |
| B2 | `plugins/branches/hooks/posttooluse-push-branch.sh` | 31, 42 | `tool_input.command`, `session_id` | push não é notado; o aviso de branch some |
| B2 | `plugins/context-guard/hooks/context-guard.sh` | 33 | `session_id` | o guard de contexto não abre |
| B2 | `plugins/context-guard/hooks/context-guard-reset.sh` | 18 | `session_id` | o sentinel da sessão nunca é limpo |
| B2 | `plugins/context-guard/hooks/context-guard-writer.sh` | 25 | `session_id` | o `%` de contexto nunca é gravado — e o guard depende desse arquivo |
| B2 | `plugins/intent-guard/hooks/mark-work.sh` | 21 | `session_id` | o trabalho da sessão não é marcado; `delivery-audit` fica sem base |
| B2 | `plugins/lixeiro/hooks/posttooluse-anota.sh` | 34, 37 | `tool_input.command`, `session_id` | artefato criado não é anotado |
| B2 | `plugins/lixeiro/hooks/sessionend-colhe.sh` | 31 | `session_id` | a colheita de fim de sessão não acontece |
| B2 | `plugins/lixeiro/hooks/sessionstart-orfaos.sh` | 32 | `session_id` | órfãos de sessões passadas não são cobrados |
| B2 | `plugins/lixeiro/hooks/stop-colhe-turno.sh` | 39, 40 | `stop_hook_active`, `session_id` | sem o `stop_hook_active` não há nem guarda de reentrância nem colheita |
| B2 | `plugins/project-skills/hooks/posttooluse-doc-read.sh` | 24 | `session_id` | a leitura de doc não é registrada; o `doc-guard` segue cobrando |
| B2 | `plugins/project-skills/hooks/sessionstart-doc.sh` | 28 | `session_id` | o briefing de doc não abre a sessão |
| B2 | `plugins/project-skills/hooks/posttooluse-andamento.sh` | 49, 74 | `session_id`, `tool_input.command` | a linha de andamento do motor nunca sai na barra |
| B2 | `plugins/project-skills/hooks/stop-doc-touch.sh` | 22, 24 | `session_id`, `stop_hook_active` | a cobrança de doc defasada não sai |
| B2 | `plugins/project-skills/hooks/userpromptsubmit-plan-escape.sh` | 46 | `session_id` | a escapatória do gate de plano não é registrada |
| B2 | `plugins/project-skills/hooks/sessionstart-plan.sh` | 28 | `session_id` | o plano aberto não ressuscita depois do `/clear` |
...
| B2 | `plugins/project-skills/hooks/stop-plan-status.sh` | 38, 41 | `stop_hook_active`, `session_id` | o status do plano não é cobrado no fim do turno |
| B2 | `plugins/gauntlet/hooks/sessionstart-lembra-missao.sh` | 29 | `session_id` | o lembrete da missão aberta não é injetado no arranque — **e este é o único da classe B que ainda sairia CALADO**: ele não chama `hj_avisa`, e por isso o `test_sem_jq.sh` reprova hoje |
