...
- [confirmado, derivado nesta rodada] **Os campos saíram do papel: 330 tarefas nos 25 planos trazem `requisito` e `pronto`, 11 arquivos têm o bloco `requisitos` no topo e 31 tarefas têm `decidido`.** A exposição descrita acima deixou de ser só do formato — é conteúdo de hoje, e é conteúdo sem cópia. Seguem em zero o `grupo` e a `pendencia`.
...
- [confirmado, medido nesta rodada] O depósito cresceu de 13 para **25** planos (**644K**), e hoje há **dois** `status: "active"` (`2026-08-06-a-metodologia-vira-mecanismo` e `autopsia-2026-08-09`) — o impasse dos dois ativos simultâneos voltou a ser o estado deste disco. ⚠️ Outros **quatro** arquivos estão abertos sem status nenhum, e o que se perde com eles é o mesmo: nada os copia.
- Nada disso é cobertura: protege contra o plano *virar outro*, não contra o arquivo sumir.
- Coberto por teste [confirmado, rodado nesta sessão]: `python3 plugins/project-skills/lib/test_plan_state.py` → `OK` (**322** asserções `ok`, contra 318 na rodada anterior; a última impressa é `declarar a seção vazia apaga de propósito`) e `python3 plugins/project-skills/lib/test_cobertura.py` → `OK` (**73** asserções `ok`, contra 68; a última impressa é `e no plural`).

#### 3.5.1 · O calculador do fio — `plugins/project-skills/lib/cobertura.py`

- [confirmado] **NÃO é depósito.** 349 linhas nesta rodada (nasceu com 79), nenhuma escrita em disco. Lê um markdown de requisitos — e os outros documentos de régua, por `le_jornadas`, `le_artigos`, `le_pecas` e `le_passos` —, cruza com o plano e devolve os quatro estados do fio (coberta · sem requisito · requisito órfão · citação inexistente) mais os cruzamentos com a lei, as jornadas, as peças e os passos. A lista viva dos baldes sai do programa: `python3 -c "import cobertura; print(sorted(cobertura.mapa({}, {})))"` de dentro de `plugins/project-skills/lib/`.
