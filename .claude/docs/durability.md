...
doc-sig: pedro-plugins/.gitignore@gen=3.8#1f30f624
---
...
- [confirmado, remedido nesta rodada] `.claude/plans/` ignorado por `.gitignore:18`. **24** arquivos no repo (**636K**, 558.858 bytes de JSON); `~/.claude/plans/` está em **1,1M** — era 2,3M na rodada anterior, e a queda é o encolhimento do harness, não apagamento nosso.
...
- [confirmado, derivado nesta rodada] **Os campos saíram do papel: 330 tarefas nos 24 planos trazem `requisito` e `pronto`, 11 arquivos têm o bloco `requisitos` no topo e 31 tarefas têm `decidido`.** A exposição descrita acima deixou de ser só do formato — é conteúdo de hoje, e é conteúdo sem cópia. Seguem em zero o `grupo` e a `pendencia`.
...
- [confirmado, medido nesta rodada] O depósito cresceu de 13 para **24** planos (**636K**), e hoje há **um** `status: "active"` (`2026-08-06-a-metodologia-vira-mecanismo`) — o impasse dos dois ativos simultâneos deixou de ser o estado deste disco. ⚠️ Outros **quatro** arquivos estão abertos sem status nenhum, e o que se perde com eles é o mesmo: nada os copia.
