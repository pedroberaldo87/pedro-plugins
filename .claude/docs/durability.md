...
- Coberto por teste [confirmado, rodado nesta sessão]: `python3 plugins/project-skills/lib/test_plan_state.py` → `OK` (**318** asserções `ok`, contra 173 na rodada anterior; a última impressa é `declarar a seção vazia apaga de propósito`) e `python3 plugins/project-skills/lib/test_cobertura.py` → `OK` (**68** asserções `ok`; a última impressa é `e no plural`).
