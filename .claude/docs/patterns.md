...
- **Fora do gate por desenho: `scripts/anti_slop_inventario.py`** — **novo em 2026-08-19**. Ele MEDE e LISTA, não conserta e não reprova commit nenhum: varre o que o git rastreia (menos `graphify-out/`, menos `.claude/reports/` e menos ele mesmo, porque medir o próprio relatório faria a contagem se citar) e devolve o inventário das quatro classes da mesma doença — **caminho de doc cravado**, **lista duplicada**, **contagem escrita à mão**, **valor copiado** —, cada classe com contagem, dono (a fonte única que deveria mandar) e de-para. Cada classe traz o comando que reconta a si mesma, então o inventário não vira número envelhecendo na prosa [confirmado nesta rodada — `python3 scripts/anti_slop_inventario.py` → `varredura: 653 arquivos rastreados`, com `A · caminho de doc cravado` em `719 ocorrências em 147 arquivos`, rc=0]. Suíte própria: `scripts/test_anti_slop_inventario.py` (entra na esteira pelo glob `scripts/test_*.py` do check J). ⚠️ **Cobrador, nenhuma das quatro classes tem ainda** — é dívida declarada, e o relatório da varredura mora em `.claude/reports/`, que é gitignorado (registro de trabalho não é produto).
