...
- **project-doc** `v3.6.0` — Gera sistema de documentação a partir de TODA a evidência (arquivos, handoffs, memory, grafo, git log, transcripts), num journal append-only versionado, projetado em índice `CLAUDE.md` + `.claude/docs/*.md` + ponteiros finos. Scrubber move segredos pro cofre. FULL/`--deep` mineram via Workflow (grafo é premissa) e LEEM o código real por fan-in do grafo; auto commit+push escopado dos artefatos de doc no fim. **Detecção de padrão (gen 3.6):** `pattern_check.py` verifica invariantes estruturais NO DISCO (markers v2, frontmatter, journal, doc-sig, gen) + assinatura determinística por doc (sha256 do corpo) — não confia só no número do `gen`. Modo `--nested` experimental (ponteiro aninhado por app, opt-in). `category: productivity`. ⚙️ hooks.
...
- **Detecção de padrão (`plugins/project-doc/lib/pattern_check.py`, gen 3.6).** Stdlib-only. `check_pattern(project_root)` verifica invariantes estruturais NO DISCO (markers v2 abertura+fechamento; frontmatter em todo doc; journal presente; linha `doc-sig` no frontmatter; `gen == CURRENT_GEN`) e devolve `{in_pattern, gen_found, violations, docs}` — o passo 0.1 da skill e o check #19 da Verification consomem isso (não confiam só no número do `gen`, que pode mentir). Assinatura determinística por doc: `<project>/<scope_basename>@gen=<gen>#<hash8>` onde `hash8 = sha256(corpo após frontmatter)[:8]`. CLI: `--project-root [--json]` e `--sig <doc>`. Consumido pelos hooks (`doc-detect.sh` 5ª coluna `out_of_pattern`; `sessionstart-doc.sh` e `pretooluse-doc-guard.sh` avisam fora-do-padrão). Degrada gracioso (python/lib ausente → `out_of_pattern=0`, fail-open).
...
- **Detecção de padrão verifica o disco, não só o `gen` (project-doc gen 3.6)** — o número do `gen` no marker pode mentir (editado à mão / estrutura incompleta), então `pattern_check.py` checa invariantes reais no disco; o `gen` bumpa em TODA mudança estrutural do padrão de doc. A liberação do guard doc-first é por **sentinel-de-disco escrito quando a doc é LIDA** (`posttooluse-doc-read.sh`), não por assinatura-no-transcript — esta última é estruturalmente inviável pro `CLAUDE.md` raiz, que não carrega a própria assinatura no corpo.
...
>
> **`apply-config` faz MERGE, não overwrite cego** (`apply-config.sh:10-13,51-55`): `env` → os defaults vencem em conflito; `permissions.allow`/`permissions.deny` → UNIÃO (a máquina mantém as suas + ganha as defaults, nunca remove permissão existente); `permissions.defaultMode` → mantém o local, senão o default; `settings.local.json` fica **intocado** (pode ter secrets). Garante idempotência sem pisar em config de máquina.
