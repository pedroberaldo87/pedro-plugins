...
- ⚠️ **Editar `_shared/` sem rodar `scripts/sync-shared.sh`** deixa as 106 cópias vendoradas defasadas (o número sai do mapa, nunca daqui: `sed -n '/^SPECS=(/,/^)/p' scripts/sync-shared.sh | grep -c '::'`) — fix de código compartilhado nasce em `_shared/`, nunca na cópia.
...
- **[architecture.md](.claude/docs/architecture.md)** — estrutura do repo, anatomia de plugin, catálogo dos 23 plugins distribuídos com versões, os 12 plugins com hooks evento a evento, a engine vendorada (`_shared/` → 106 cópias em 44 pastas, §7), decisões de arquitetura, terceiros do bootstrap
