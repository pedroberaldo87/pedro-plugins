...
- ⚠️ **Editar `_shared/` sem rodar `scripts/sync-shared.sh`** deixa as 95 cópias vendoradas defasadas (o número sai do mapa, nunca daqui: `sed -n '/^SPECS=(/,/^)/p' scripts/sync-shared.sh | grep -c '::'`) — fix de código compartilhado nasce em `_shared/`, nunca na cópia.
