...
doc-sig: pedro-plugins/.gitignore@gen=3.8#58e4b62b
---
...
### A5d · Os quatro retratos NOVOS — a família de baselines virou seis (e sete no disco)
...
**O sétimo chegou em 2026-08-21, e ainda não é rastreado: `.claude/artigo8.baseline.json`** — **277 achados, ~100K**, e é o maior arquivo da família. Congela a dívida do Artigo 8 (comando de `SKILL.md` que só roda na árvore de quem escreveu), lido pelo check **V** do `release-gate.sh`, que reprova só o achado NOVO. As chaves são `skills` (quantas a varredura alcançou) e `findings`. ⚠️ **Enquanto ele não entrar no índice, ele é retrato desta máquina**: `git ls-files '.claude/*.baseline.json'` devolve seis, e num clone sem o arquivo o `--check` sai 0 dizendo *"retrato ausente ou ilegível — nada a comparar"* — ou seja, o check V passa verde sem medir nada. O `.gitignore` não o exclui, então o commit desta onda é o que fecha o buraco [confirmado nesta rodada — `git check-ignore .claude/artigo8.baseline.json` não casa; `git ls-files '.claude/*.baseline.json'` → 6 caminhos; `python3 -c "import json;d=json.load(open('.claude/artigo8.baseline.json'));print(len(d['findings']))"` → `277`].
