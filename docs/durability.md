...
doc-sig: pedro-plugins/.gitignore@gen=3.8#dee09afd
---
...
- [confirmado, remedido nesta rodada] `.claude/plans/` ignorado por `.gitignore:18`. **25** arquivos no repo (**644K**, 566.589 bytes de JSON); `~/.claude/plans/` está em **1,1M** — era 2,3M na rodada anterior, e a queda é o encolhimento do harness, não apagamento nosso.
...
- [confirmado, derivado nesta rodada] **Os campos saíram do papel: 568 tarefas (de 744) nos 31 planos trazem `requisito` e `pronto`, 17 arquivos têm o bloco `requisitos` no topo e 63 tarefas têm `decidido`.** A exposição descrita acima deixou de ser só do formato — é conteúdo de hoje, e é conteúdo sem cópia. O `grupo` (58) e a `pendencia` (10) saíram do zero nesta rodada, e entram na mesma exposição.
...
- ⚠️ **O teto vem de fora do módulo** (`from regua_texto import BULLET_MAX`, e `DESC_MAX = BULLET_MAX`), da cópia vendorada que mora ao lado do `plan_state.py`, em `plugins/project-skills/lib/` — uma das **10** que o `sync-shared.sh` mantém. Perder a cópia não perde plano, mas faz o `plan_state.py` parar de importar: a régua compartilhada virou dependência de execução do depósito, não só do renderizador. [confirmado]
...
- [confirmado, medido nesta rodada] O depósito cresceu de 25 para **31** planos (**775K**), e hoje há **um só** `status: "active"` (`2026-08-12-os-quatro-itens`) — o impasse dos dois ativos simultâneos, que era o estado do disco na rodada anterior, se desfez (20 `done`, 6 `abandoned`). ⚠️ Outros **quatro** arquivos continuam abertos sem status nenhum (`2026-08-05-constituicao-e-onda-0`, `2026-08-05-portabilidade-windows`, `2026-08-06-doc-em-apresentacao`, `2026-08-06-os-quatro-issues-que-sobraram`), e o que se perde com eles é o mesmo: nada os copia.
- Nada disso é cobertura: protege contra o plano *virar outro*, não contra o arquivo sumir.
- Coberto por teste [confirmado, rodado nesta sessão]: `python3 plugins/project-skills/lib/test_plan_state.py` → `OK` (**348** asserções `ok`, contra 322 na rodada anterior; a última impressa é `plano 'done' com todo passo provado passa`) e `python3 plugins/project-skills/lib/test_cobertura.py` → `OK` (**73** asserções `ok`, contra 68; a última impressa é `e no plural`).

#### 3.5.1 · O calculador do fio — `plugins/project-skills/lib/cobertura.py`

- [confirmado] **NÃO é depósito.** 394 linhas nesta rodada (nasceu com 79), nenhuma escrita em disco. Lê um markdown de requisitos — e os outros documentos de régua, por `le_jornadas`, `le_artigos`, `le_pecas` e `le_passos` —, cruza com o plano e devolve os quatro estados do fio (coberta · sem requisito · requisito órfão · citação inexistente) mais os cruzamentos com a lei, as jornadas, as peças e os passos. A lista viva dos baldes sai do programa: `python3 -c "import cobertura; print(sorted(cobertura.mapa({}, {})))"` de dentro de `plugins/project-skills/lib/`.
...
- **O que MAIS mora aqui desde 2026-08-09, e não é da mesma classe:** `ativo-*` (só o do motor `gauntlet`), `sinal-*`, `placar-*`, `trabalho-*`, `doc-*` e `onda-*` passaram a nascer nesta pasta na casa única (§3.15b). São **efêmeros por sessão**: perder não custa nada, e a próxima missão os recria. A pasta ficou, portanto, **mista** — um ativo insubstituível ao lado dos descartáveis, no mesmo `rm -rf`.
- **`onda-<sid>` (2026-08-09) entra como efêmero, e a perda dele é invisível de propósito.** Ele guarda a rodada em curso e o par feitos/total do plano só para a barra de status desenhar (`data-stores.md §B13`); o dado de verdade continua sendo o `.plan.json` versionado no repositório (§3.4), de onde o número é recontado a cada marcação. Apagar tira uma informação da barra até o próximo `tick`, e nada além disso. **RPO: total. RTO: a próxima marcação.**
...
- 🔴 **E o SOBRAR era o risco real desta pasta — não o perder.** A expiração por idade existia, mas quem a disparava era o gate no `pretooluse`: ela só rodava **quando alguém da própria sessão consultava**, e sessão morta não consulta mais. Medido em 2026-08-10: **cinco `ativo-*` órfãos vivos ao mesmo tempo, o mais velho de 75 horas**. Desde então a varredura roda também no desenho da barra (`andamento.py:expira_sinais`, chamada de `linha_motor`), que é o único ponto com frequência garantida em toda sessão viva — e ela apaga o **conjunto** da sessão vencida, não só o sinal, porque estado sem dono reaparece na barra de quem reusar o id. É a única "cobertura" desta pasta, e ela **limpa**, não copia: a natureza do dado é efêmera. [confirmado — os cinco morreram na primeira leitura, com o autor `barra` no `expirados.log`]
- **`motorid-<sid>` (2026-08-12) entra como efêmero, e a perda dele agora tem consequência declarada.** Ele lista quem está de pé na sessão (`dono\tmotor`, uma linha por motor) e é o que faz o `encerra` saber que ainda há missão viva antes de apagar o sinal. **Perder = o sinal volta a cair no primeiro `encerra`**, que é o defeito de 2026-08-12 reaparecendo — a barra fica muda com trabalho rodando. A rede embaixo é `andamento.py:ressuscita_sinais`, chamada no mesmo desenho da barra: sinal ausente com motor registrado é **reaceso**, e registro mais velho que `SPRINT_TTL_MIN` é apagado em vez de reacender para sempre. **RPO: total. RTO: o próximo `arma`.** [confirmado — `test_andamento.py`, bloco "aviso apagado com execução viva é REACESO pela varredura da barra"]
- **RPO: total. RTO: semanas de uso** — para o `duracoes-*.json`. Apagar não quebra funcionalidade — o narrador do fluxo 19 volta a sair só com o relógio, sem estimativa —, mas a memória recomeça do zero e só volta com o tempo. Para os efêmeros, RTO é a próxima missão.
...
- Natureza: append-only com lock. `append()` só escreve; `fold()` deriva o estado vivo; `locked()` faz o load+append atômico, evitando ids `r-N`/`p-N` duplicados quando hooks concorrentes chamam ao mesmo tempo.
- ⚠️ **A trava passou a ter duas implementações e um teto, em 2026-08-10, e isso muda o que ela GARANTE.** Onde há `fcntl` (POSIX) é `flock(LOCK_EX)`, exclusão de verdade. Onde não há — Windows —, é um diretório `ledger.lock.d` criado com `os.mkdir`, atômico em qualquer sistema de arquivos. **Nos dois caminhos a espera tem teto de 5 s, e estourado o teto o programa segue SEM a trava**: a escolha está escrita no código (*id duplicado é incômodo visível, missão pendurada é dano*) e o efeito de durabilidade é que **id duplicado é desfecho possível**, não impossível — sob concorrência alta e sustentada, o append continua, o arquivo não corrompe, mas dois eventos podem nascer com o mesmo `r-N`. Quem lê o ledger não pode assumir id único como invariante forte.
...
- **O vizinho `paradas.jsonl` (mesma pasta, mesmo gitignore, `data-stores.md §A11b`) tem a MESMA cobertura: nenhuma, POR DECISÃO — e perde mais.** [confirmado — `ledger_corridas.py:caminho_paradas` escreve, `le_paradas` lê, e nada além deles toca o arquivo; quantas linhas hoje: `wc -l < .claude/.sprint/paradas.jsonl`] Ali mora uma linha por parada do laço com `causa`, `conserto` e o `sha` do commit; o commit sobrevive no git, mas a associação *pedra → conserto → corrida* só existe neste arquivo, e é dela que a seção de problemas do relatório final é derivada. Apagado, a história das paradas some e o relatório volta a sair da memória da sessão — que é o defeito que o registro existe para matar; o plano em si não depende dele. Fica fora do índice pela mesma regra do resto da pasta: registro de trabalho não é produto (`.gitignore:34`).

### 3.16g · A prova das quatro passadas da largada — `<projeto>/.claude/.sprint/precheck.json`

Entrou no inventário em 2026-08-21. Escrito por `plugins/project-skills/lib/precheck_largada.py` (`--relatorio`), lido pelo `--confere` antes da largada do `/sprint`.

- **Cobertura: NENHUMA, por decisão — e aqui a ausência é a resposta certa, porque o depósito é efêmero por NATUREZA.** [confirmado — gitignorado com a pasta inteira (`.gitignore:34`), e nesta rodada o arquivo nem existe no disco] Ele é a prova de que as quatro passadas do pré-check rodaram sobre ESTE plano; perder custa exatamente um comando: rodar `--relatorio` de novo.
- **O próprio programa o descarta quando o mundo muda.** A `marca` gravada cobre os passos abertos do plano + o registro selado; plano reescrito, passo novo ou decisão nova vencem o relatório, e o `--confere` recusa a largada até um `--relatorio` fresco. Um artefato desenhado para vencer sozinho não é candidato a backup — cópia velha dele seria pior que ausência, porque provaria uma conferência que já não vale. [confirmado — `precheck_largada.py:marca` e `confere_largada`]
...
### 3.19 · Saída do `/visual`, do qa-loop e da vistoria no repo — `.claude/visual/`, `.claude/qa-loop/`, `.claude/vistoria/`
...
- **O vizinho `.claude/vistoria/` (108K, 5 arquivos, `.gitignore:20`) é o mesmo caso com UMA exceção.** As três páginas e o JSON de achados voltam rodando `medidor.py --json | pagina.py --dir .claude/vistoria`; o `piloto-leitor-2026-08-09.json`, não — ele é a rodada-piloto do leitor por agente, julgamento escrito à mão, e nenhum comando o refaz. Cobertura: nenhuma, para os dois. [confirmado — `git check-ignore -v .claude/vistoria/` → `.gitignore:20`, e o campo `_piloto` do JSON]
