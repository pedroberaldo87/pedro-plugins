...
doc-sig: pedro-plugins/release-gate.sh@gen=3.8#836d6784
---
...
**Quarta aplicação, 2026-08-21 (pente fino), e ela fecha a porta de entrada: payload sem `session_id` → `exit 0` liberado.** O padrão antigo era `hj_campo_ou "$INPUT" session_id unknown` — e `unknown` é UM sentinela para todas as sessões sem id, ou seja, o bug original com outra roupa. Hoje a convenção é `SESSION=""` + `[ -n "$SESSION" ] || exit 0`, com o comentário-padrão no lugar (*"o sentinela \"unknown\" seria compartilhado entre sessões (o defeito do context-guard v1.1)"*) — e a contagem de hooks sai dele, nunca daqui: `grep -rl 'sentinela "unknown" seria compartilhado' plugins/*/hooks/*.sh | wc -l` → **12** nesta rodada [confirmado, executado].
...
**A FORMA da recusa também é convenção desde 2026-08-21 (pente fino): cabeçalho + bullets, no perfil `hook` (140 por linha).** As recusas em parágrafo corrido foram reescritas nos hooks que negam (`pretooluse-plan-gate.sh`, `scope-cop.sh`, …): a 1ª linha com emoji diz O QUÊ, e cada bullet diz um porquê ou uma saída. O cobrador é novo e mede o lado que o check I nunca viu — o SHELL: `scripts/test_regua_hook_msgs.py` varre toda linha de `plugins/*/hooks/*.sh` e `.claude/hooks/*.sh` que emite mensagem (`hj_deny`/`hj_block`/`hj_msg_ctx`/`systemMessage`/atribuição de `MSG=`…), tira as expansões `$…` do texto literal entre aspas, quebra nos `\n` e cobra teto **160** por linha emitida — o 140 do perfil + folga de marcação, porque expansão removida e marcação não contam na régua. Isenção na linha: `msg-ok: <motivo>`. Entra na esteira pelo glob `scripts/test_*.py` do check J, e o que ele declara NÃO pegar está na própria docstring (string multilinha e mensagem concatenada em variável fora do padrão) [confirmado — `python3 scripts/test_regua_hook_msgs.py` → `OK — 123 hooks medidos, nenhuma mensagem acima do teto`, rc=0].
...
**Três frentes cobram a régua hoje**, e nenhuma sozinha fecha: o check I do release-gate barra **gerador de HTML que não a chama** (§5.2), os dois hooks do §1.17 pegam o texto já escrito — o arquivo na porta, o relato na rede — e `scripts/test_regua_hook_msgs.py` mede a mensagem literal dos hooks shell (§1.5a), o lado que o check I não alcança.
