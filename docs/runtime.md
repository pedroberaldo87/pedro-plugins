...
doc-sig: pedro-plugins/sessionstart-doc.sh@gen=3.8#57c58275
...
### 1d · Permissões só entram com CONSENTIMENTO gravado — o portão do `apply-config.sh`

**Não é passo do ciclo acima**: `hooks/lib/apply-config.sh` roda sob demanda, chamado pelo `/bootstrap:setup` (o cabeçalho do arquivo diz "run on demand, once per new machine"), nunca pelo `session-sync.sh`. Desde o pente-fino de 2026-08-21 (`bfbe936`) ele tem um portão antes do merge de settings:

- **A marca** é `<CLAUDE_CONFIG_DIR>/pedro-plugins-permissions-ok` (default `~/.claude/`). Sem ela, o script **recorta `permissions` dos defaults** (gera `.defaults-sem-permissoes.json` temporário, apagado depois) e aplica só env + flags + statusLine, avisando: *"permissões NÃO aplicadas — falta o consentimento"*. O racional está no comentário: o allow dos defaults **liga aprovação automática** na máquina de quem instala, e default de risco não nasce ligado (Artigo 2 da constituição).
- **Quem grava a marca é o `/bootstrap:setup`**, e só depois de mostrar ao usuário o que o allow liga — a contagem sai de um comando sobre `config/settings-defaults.json` escrito na própria SKILL.md, nunca de número em prosa.
- **Anistia para máquina já mesclada**: se o `settings.json` já contém uma permissão distintiva dos defaults (o grep é por uma chamada do Playwright), o merge anterior é a prova do estado, a marca é gravada automaticamente com log `anistia: merge anterior detectado` e o fluxo segue — travar agora só a deixaria sem atualização.

`[confirmado — leitura do bloco de consentimento em apply-config.sh e da seção nova em skills/bootstrap/SKILL.md]`
...
**Verificado:** `bash plugins/bootstrap/hooks/test_bootstrap_hooks.sh` → **33 ok · 0 FAIL** neste run, incluindo o round-trip do snapshot ("pedro-plugins continua com 23 plugins", "2a rodada e idempotente") e os dois checks do portão de consentimento ("sem a marca, o allow não é aplicado" · "com a marca, o allow dos defaults entra inteiro"). `[confirmado — saída da suíte nesta rodada]`
...
3. **Leitura** — `context-guard.sh` lê `/tmp/claude-context-pct-<session_id>`, trunca o decimal (`PCT_INT="${PCT%.*}"`) e compara com `CLAUDE_CONTEXT_THRESHOLD` (default `80`). ⚠️ Payload sem `session_id` → `exit 0` liberado desde 2026-08-21: o fallback `unknown` fazia sessões sem id dividirem o MESMO sentinel — o defeito da v1.1 renascido, agora extinto na família inteira (ver a nota do fluxo 7). `[confirmado]`
...
**E desde 2026-08-21 o mesmo relatório passou a guardar o TIQUE — o portão chegou ao `plan_state.py`.** `cmd_tick` chama `_gate_precheck` antes de marcar qualquer passo: relatório **vencido** (o plano mudou desde a medição) recusa o tique, com a mesma régua de frescor da largada — `confere_largada` **importada** de `precheck_largada.py`, nunca copiada, porque régua de frescor é UMA e é de quem mediu. Plano **sem nenhum relatório no disco** passa com aviso no stderr — **fail-open declarado**: plano velho não ganha portão retroativo, e a mensagem já traz o comando que gera o relatório. O escape é `PLAN_STATE_PRECHECK=off`, sempre dito no stderr, nunca em silêncio — o molde é o kill-switch do plan-gate (`MODE_FILE = "off"`), trocado por variável de ambiente porque o tique roda por CLI, não por hook. ⚠️ **E depois do save o portão RESSELA a marca sobre o plano já ticado**: a marca cobre só os passos ABERTOS, então o passo que o próprio tique fecha sairia da lista e o relatório venceria por conta do próprio andar — resselar é o que torna o frescor imune ao próprio progresso. `[confirmado — `plan_state.py:_gate_precheck` (F22.10 · R-32), chamado em `cmd_tick` com o resselador rodando depois do save, e o bloco "portão do pré-check no tick" de `test_plan_state.py`, que saiu OK nesta rodada]`
...
**A missão ganhou VIGÍLIA, e ela nasce e morre com o sinal do motor.** No bloco 1, logo depois do `andamento.py arma` e **antes** de chamar o `Workflow`, a casca acende o `/goal` do harness com a condição desta missão (o plano fecha; cada parada é investigada, consertada e relançada por `lib/retomada.py`; só a lista fechada de casos do dono interrompe o laço); no bloco 2, junto do `encerra`, ela **apaga** — em todo desfecho de parada, obra pronta inclusive, porque vigília acesa depois do motor morto relança missão que já acabou. A vigília não é comando avulso nem opção do dono: *sprint que para na primeira parada não é sprint, é uma rodada só*. ⚠️ **E o laço não pode remendar antes de apurar:** a causa só existe com a **saída crua do comando** que a mostra, colada literal (memória do que aconteceu não é prova), e essa causa provada ainda vai a um **desafiador** com a ordem de derrubá-la — desafiador mudo não referenda, e três voltas sem acordo fazem o caso subir ao dono (`retomada.py --caso causa-repetida`), nunca ao remendo. Faltando qualquer um dos dois passos, o laço **não conserta e não relança**. ⚠️ **Máquina sem o `/goal` instalado não finge que vigia** (2026-08-21): a casca declara a ausência em voz alta duas vezes — na largada (*"vigília sem /goal — rito manual"*) e de novo no relatório final — e no lugar do goal entra o **rito manual**, os mesmos passos sem afrouxar nada: a cada parada, investigar → causa com prova de comando → desafiador → conserto commitado → a linha da parada no ledger → relançar. No encerramento não há goal para apagar, mas a declaração da ausência segue viva até o relatório; silenciá-la, ou seguir sem o rito fingindo que vigia, é o único desfecho proibido. E quando o motor **já morreu** e o dono volta (a manhã seguinte), quem rearma o laço é a skill `/reentrada` do mesmo plugin: lê o último run do ledger de corridas, passa o gate do `relance` (causa repetida vira caso do dono), classifica pela mesma `lib/retomada.py` e **aponta** os blocos da SKILL.md do sprint que relançam — nunca copiando os blocos, e nunca com motor ainda vivo (motor de pé se observa com `/monitorar`). `[confirmado — o bloco 1 e o bloco 2 de `skills/sprint/SKILL.md` (inclusive o trecho "GOAL AUSENTE NA MÁQUINA"), os checks `F23.3`/`F23.4` de `test_sprint_skill.py`, o check `F23.8` de `test_retomada.py` (→ `OK: retomada (10 desfechos…)`) e `test_reentrada_skill.py` → `4 casos, tudo verde`, rodados nesta revisão]`
...
⚠️ **Payload sem `session_id` LIBERA — o sentinela "unknown" cross-sessão morreu em 2026-08-21** (`bfbe936`). Todo hook que chaveia estado por sessão saía do impasse com o fallback `unknown`, e aí TODA sessão sem id no payload dividia o mesmo sentinel — aviso dado numa calava as outras, o mesmo defeito que derrubou o context-guard v1.1. Agora esses hooks saem `0` calados quando o campo falta, com o comentário idêntico na linha da guarda. Quantos e quais sai do grep, nunca desta lista: `grep -rl 'payload sem sessão: liberado' plugins/*/hooks/*.sh | wc -l` → **12** neste run (o gate de plano, o guard de doc, o gate de organismo, o registrador de leitura de doc, o context-guard, o graphify-guard, o `branches`, os dois `sessionstart-*` de doc e plano, os dois `stop-*` de doc-touch e status de plano, e o escape verbal do plano). `[confirmado — a contagem acima e o diff dos 12]`
...
4. **`handoff/sessionstart-ata.sh`** — não injeta contexto. Grava `/tmp/claude-ata-session-<sha1(cwd)[:12]>` com `{session_id, transcript_path, cwd, source}`, porque a skill `handoff` não recebe `session_id` (skill ≠ hook) e o `extract_ata.py --auto` precisa do sentinel pra achar o `.jsonl` certo. O hash tem que ser idêntico ao de `extract_ata.py`. Do outro lado do fio, o extrator deixou de cravar `.claude/docs/` em 2026-08-21: `extract_ata.py:estado_etapas` resolve a casa da doc pela cascata do `casa_da_doc.py` vendorado de `_shared/` — `docs/` na raiz se existir, senão `.claude/docs/`, e doc que ainda não nasceu nasce na casa canônica. `[confirmado — bfbe936 e a suíte test_handoff_skill.py]`
...
**Portas antes de qualquer julgamento:** `PLAN_DOC_GATE=0` desliga; `command -v jq` fail-open; payload sem `session_id` → `exit 0` (desde 2026-08-21 — a família inteira, ver a nota do fluxo 7); `project_root` não achou raiz → `exit 0`; **`doc-detect.sh` ilegível → `exit 0`** — helper ausente não é "projeto sem doc", é o gate cego (o comentário registra que um `chmod 000` fazia projeto documentado cair no caso A e ser negado sem cap). `[confirmado]`
...
**A recusa fala em cabeçalho + bullets desde 2026-08-21** (`bfbe936`): a 1ª linha diz o veredito (`📐 <projeto> NÃO tem documentação nenhuma …`), e cada instrução vira um bullet — o parágrafo corrido de antes enterrava a ordem no meio da prosa. A mesma reescrita valeu para o `deny` do `pretooluse-doc-guard.sh` (a mensagem "📚 … A busca fica bloqueada até a doc ser lida"). Quem cobra a forma é o cobrador novo `scripts/test_regua_hook_msgs.py`: varredura estática de toda linha de `plugins/*/hooks/*.sh` e `.claude/hooks/*.sh` que emite mensagem (`hj_deny`/`hj_block`/`systemMessage`/atribuição de `MSG`…), com teto de 160 caracteres por linha emitida (o 140 do perfil hook de `_shared/regua_texto.py` + folga de marcação) e isenção por `msg-ok: <motivo>`. Neste run: **os hooks do repositório (a contagem sai de `python3 scripts/test_regua_hook_msgs.py`, que a imprime) medidos, nenhuma mensagem acima do teto**. `[confirmado — leitura do cobrador e a rodada dele]`
...
Campos da saída: `regua`, `marca_regua`, `ausentes` (e a mesma lista separada por natureza em `ausentes_lei`, `ausentes_acordo`, `ausentes_minerados`), `dispensa`, `reabertos`, `correcoes_pendentes` — e `anexos` (novo em 2026-08-16): os sidecars de protótipo (`.claude/docs/prototipo/*.prototipo.md`, `doc_load.py:le_anexos`), quarta natureza que **nunca** entra na régua nem na `marca_regua` — o protótipo muda de tela sem mudar a lei —, cada um com os dois gatilhos de reabertura nomeados (conjunto que diverge do `conjunto-sig` gravado · `design-sig` que não bate mais com o de acordo do `anexo-de`); a lei do formato é `.claude/docs/prototipo/FORMATO.md` (§1.10a de `patterns.md`). **Ausência é dita em voz alta e não é achado** — e desde 2026-08-12 ela é dita no ALTO: o bloco `⚠️ LACUNA — N de M documentos canônicos` abre o relatório, uma linha por natureza, cada uma com o comando que a resolve (`/start escreve` · `/doc extrai do código`). Só `dispensa.md` com `motivo:` escrito cala o bloco — projeto sem `constituicao.md` não tem o eixo de constituição, e isso não viola nada; `dispensa.md` sem `motivo:` escrito não vale. ⚠️ **O denominador M deixou de ser fixo em 2026-08-21** (`bfbe936`): `doc_load.py:tem_tela` consulta o detector de interface — a MESMA regra de `hooks/lib-has-frontend.sh` (F2.2), reimplementada em Python com a paridade cobrada por `test_doc_load.py` — e projeto **sem tela não tem `design.md` cobrado**: o campo `canonicos` da saída desce em 1 e o arquivo some da lista de ausentes (existindo, ele é lido normal). Cobrança que não cabe ensina a ignorar cobrança; e o indeterminado (sem git) cala a cobrança em vez de cobrar quem não deve.
...
**Verificado:** `python3 plugins/project-skills/lib/test_doc_load.py` → **53 passou · 0 falhou** nesta rodada · `python3 plugins/project-skills/lib/test_spec_to_plan_skill.py` → **52 ok · 0 falhas** · `python3 plugins/project-skills/lib/test_start_doc_skill.py` → **OK**, 191 checks (contagem deste run: `python3 … | grep -c '^  ok'`). `[confirmado — saída das suítes]`
...
- **Conteúdo não lido nesta rodada**, citado só pelo registro: `plugins/branches/hooks/sessionstart-branches.sh`, `plugins/handoff/hooks/handoff-completeness-gate.sh`, `plugins/intent-guard/hooks/delivery-audit.sh`, `plugins/project-skills/hooks/stop-doc-touch.sh`, `plugins/guardrails/hooks/askq-humanize.sh` e `plugins/bootstrap/hooks/post-plugin-command.sh` (o `apply-config.sh` saiu da lista: o bloco de consentimento dele foi lido na rodada do fluxo 1d).
