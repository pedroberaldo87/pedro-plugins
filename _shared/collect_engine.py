...
def resolve_project_root(path, stop_at=None):
    ...
    É o que distingue projeto de agrupador: PROJETOS/ e CLIENTES/ não têm .git
    (sobe-se através deles), mas PROJETOS/app e CLIENTES/site têm.
    """
...
def infer_scope(edited_paths, cwd):
    ...
    return {
        ...
        # A skill usa os dois pra decidir se confirma o destino com o usuário.
...
def discover_all_transcripts(project_root):
    ...
    for slug_dir in sorted(glob.glob(os.path.join(PROJECTS_DIR, "*"))):
        ...
        for tp in sorted(glob.glob(os.path.join(slug_dir, "*.jsonl"))):
            cwd = _first_cwd(tp)
            if cwd is not None:
                # cwd legível: entra se resolve PRA ESTE projeto, OU se o cwd aponta
                # pra um path que não existe mais (resolve None) — transcript migrado
                # de um repo antigo cujo diretório mudou de nome/lugar (ex.: monorepo
                # que absorveu N repos: o slug já casa este projeto, mas o cwd gravado
                # no record é o path pré-migração, morto). Só descarta quando o cwd
                # resolve pra OUTRO projeto VIVO (irmão real de nome parecido).
                rp = resolve_project_root(cwd)
                if rp == project_root or rp is None:
                    out.append(tp)
                else:
                    skipped += 1
            elif is_exact:
