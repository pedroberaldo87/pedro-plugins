...
**É TUDO Opus 5.** Nenhuma etapa roda em modelo mais barato — o que varia entre elas é só o
`effort`. Decisão de projeto (2026-07-26): os dois motores rodam com o usuário ausente do loop
(`/sovai` totalmente, `/qa-loop` entre relatórios), e execução barata custa mais em
retrabalho do que economiza em token.
...
| Execução das tarefas | Opus | `high` | `executor_model` |
| Operações mecânicas e bem delimitadas | Opus | `medium` | `mechanical_model` |
...
- **`executor_model`** (Opus `high`) — execução das tarefas, uma por vez (ou em paralelo quando há independência real). Tarefa padrão (`complexity` ausente ou `'standard'`).
- **`mechanical_model`** (Opus `medium`) — operação bem delimitada (renomear, mover arquivo, 1 config, 1 valor — sem julgamento amplo): `complexity: 'mechanical'`. Mesmo modelo, **effort** mais baixo — o que cai é a profundidade de raciocínio, não a capacidade.
