...
**De:** uma sessão longa de análise de dados num projeto real, 2026-06-18, onde o usuário pediu uma avaliação meta do handoff.
**Gatilho:** o usuário perguntou "de 0 a 10, quanto o HANDOFF.md é um PRD do que tem que ser feito?". Veredito: **6/10 como PRD do trabalho restante · 9/10 como briefing de continuidade.** Este doc transforma esse veredito em ações.
...
- `user_directive` (fala do usuário) — `extract_ata.py:175`
...
## 3. A decisão de design que vem ANTES dos actionables (cravar com o usuário)
...
- **Ação:** no RESUME, adicionar passo: *"para cada item de Próximos Passos, valide que os 5 campos estão presentes; se faltar, faça a arqueologia ANTES de executar (não execute a partir de menção)."* Conecta com a regra "recuperar decisões antes de decidir".
...
- **Não** quebre o `is_human_prompt`/`SYSTEM_TAG_PREFIXES` (`extract_ata.py:40-45,126-134`) — é o que separa fala real do usuário de injeção de sistema (slash commands, system-reminders). Mexer ali contamina o LOG.
- **Não** edite o LOG à mão (regra do rito) — o LOG é da máquina; só o PRD é escrito na sessão.
...
- **A régua (por que isso importa):** decisão nunca sai de resumo comprimido — antes de executar trabalho continuado, faz-se a arqueologia do raciocínio. Um PRD com prospecto fraco força essa arqueologia a ser do zero; com A1-A2 ela vira só validação. O custo do prospecto fraco recai inteiro no RESUME.
- **O caso que originou:** análise de dados com entregável pra cliente, "Rodada 2 dos confounds". O handoff dessa sessão é um bom espécime pra testar as melhorias (Próximos Passos real, magro).
