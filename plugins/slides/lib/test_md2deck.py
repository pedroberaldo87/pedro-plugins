...
r = subprocess.run([sys.executable, os.path.join(HERE, "md2deck.py"), longo,
                    "--out", alvo], capture_output=True, text=True, encoding="utf-8", errors="replace", stdin=subprocess.DEVNULL, start_new_session=True)
...
r = subprocess.run([sys.executable, os.path.join(HERE, "md2deck.py"), prosa,
                    "--out", alvo_prosa], capture_output=True, text=True, encoding="utf-8", errors="replace", stdin=subprocess.DEVNULL, start_new_session=True)
...
src, out, pl, n = build_tmp()
r = subprocess.run([sys.executable, CHECK_FID, out, src], capture_output=True, text=True, encoding="utf-8", errors="replace", stdin=subprocess.DEVNULL, start_new_session=True)
...
with open(sujo_path, "w", encoding="utf-8") as fh:
    fh.write(sujo)
r = subprocess.run([sys.executable, CHECK_FID, sujo_path, src], capture_output=True, text=True, encoding="utf-8", errors="replace", stdin=subprocess.DEVNULL, start_new_session=True)
