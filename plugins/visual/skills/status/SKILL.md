---
name: status
description: Mostra onde a implementação do plano atual está, no CLI, em forma de árvore escaneável. Use quando o usuário digitar `/status`, ou disser "onde a gente está", "como está o plano", "o que já foi feito", "o que falta", "status da implementação". Também vale durante a execução de um motor, para acompanhar o avanço sem abrir o browser.
---
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/lib/plan_state.py render --format text --compacto
```
...
python3 ${CLAUDE_PLUGIN_ROOT}/lib/plan_state.py render <id-do-plano> --format text --compacto
```
