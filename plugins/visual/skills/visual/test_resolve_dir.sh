...
OUT_F=$(bash "$SCRIPT" "$BASE/projeto" docs/fluxos 2>/dev/null)  # casa-ok: prova o contrato do resolve-dir, cujo subdir literal e o dado do teste
if [ "$OUT_F" = "$BASE/projeto/.claude/docs/fluxos" ] && [ -d "$OUT_F" ]; then  # casa-ok: prova o contrato do resolve-dir, cujo subdir literal e o dado do teste
  ok "docs/fluxos resolve para .claude/docs/fluxos dentro do projeto"  # casa-ok: prova o contrato do resolve-dir, cujo subdir literal e o dado do teste
else
  falha "subdir com barra não resolveu na casa canônica" \
        "esperado $BASE/projeto/.claude/docs/fluxos, veio $OUT_F"  # casa-ok: prova o contrato do resolve-dir, cujo subdir literal e o dado do teste
fi

# Na reserva a barra vira traço: sem isso "claude-docs" viraria pote comum e a
# gaveta por pasta de origem se partiria em duas.
OUT_FR=$(bash "$SCRIPT" "$BASE/solto/site-do-cliente" docs/fluxos 2>/dev/null)  # casa-ok: prova o contrato do resolve-dir, cujo subdir literal e o dado do teste
case "$OUT_FR" in
