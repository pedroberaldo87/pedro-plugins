...
eq(lixeiro.classifica("/usr/bin/clang -c foo.c -o foo.o"), "efemero", "clang órfão do make é efêmero")
eq(lixeiro.classifica("javac -d out src/Main.java"), "efemero", "javac órfão é efêmero")
eq(lixeiro.classifica("xcodebuild -scheme App test"), "efemero", "xcodebuild é efêmero")
eq(lixeiro.classifica("next build"), "efemero", "build de next é efêmero")
eq(lixeiro.classifica("npx react-scripts start"), "servico", "CRA em dev é serviço")
eq(lixeiro.classifica("webpack serve --mode development"), "servico", "webpack-dev-server é serviço")
eq(lixeiro.classifica("celery -A app worker -l info"), "servico", "worker de fila é serviço")
eq(lixeiro.classifica("cargo tauri dev"), "servico", "tauri em dev é serviço")
eq(lixeiro.classifica("cargo tauri build"), "efemero", "tauri empacotando é efêmero")
eq(lixeiro.classifica("brew install postgresql@17"), "efemero", "brew instalando é efêmero")
eq(lixeiro.classifica("npx create-vite meuapp"), "efemero", "scaffolding de projeto é efêmero")
eq(lixeiro.classifica("ffmpeg -i in.mov out.mp4"), "efemero", "conversor de mídia é efêmero")
eq(lixeiro.classifica("mix test --cover"), "efemero", "suíte de elixir é efêmera")
eq(lixeiro.classifica("pnpm install"), "efemero", "instalação de dependência é efêmera")
eq(lixeiro.classifica("cargo run --bin app"), "servico", "cargo run é servidor de dev")
eq(lixeiro.classifica("python manage.py runserver 8000"), "servico", "django é serviço")
eq(lixeiro.classifica("php -S localhost:8080"), "servico", "php embutido é serviço")
eq(lixeiro.classifica("mix phx.server"), "servico", "phoenix é serviço")
eq(lixeiro.classifica("flutter run -d ios"), "servico", "flutter no simulador é serviço")
