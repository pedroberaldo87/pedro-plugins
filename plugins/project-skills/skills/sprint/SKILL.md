...
**Os três grãos medem o MESMO tripé** — qualidade · cobertura por finalidade · coerência com a
régua —, contrato em **`references/dimensoes-de-revisao.md`** (fonte:
`_shared/dimensoes-de-revisao.md` — não editar a cópia à mão; `scripts/sync-shared.sh --check` pega
...
// versão numa tabela de doc). O trabalho estava pronto no disco, o commit não passava,
// e a corrida inteira morria por isso, com o dono consertando de manhã em dois
// minutos. Morrer é a resposta certa para a causa que o motor NÃO sabe consertar; é a
...
while (!built && r < maxRounds) {
  ...
  for (const bloco of blocos) {
    ...
    // O gap do revisor de bloco SÓ reprova acima do floor — a mesma régua do revisor por
    // tarefa e do veredito final. Aqui ela faltava, e a omissão custou uma corrida inteira
    // em 2026-08-20: um gap P3 que dizia no próprio texto "Não bloqueia o commit deste
    // bloco" bloqueou o commit; os quatro revisores por tarefa tinham aprovado, `aprovadas`
    // ficou vazio, e os arquivos da onda ficaram órfãos no disco com a suíte verde.
  ...
  // nele, e achado sobre trabalho alheio vira conserto que ninguém pediu). Eixos: o TRIPÉ
  // de references/dimensoes-de-revisao.md (qualidade · cobertura por finalidade ·
  // coerência com a régua, esta pelo doc-load, citando a passagem) + spec + rastreio +
  // completude + COESÃO DO CONJUNTO. A decomposição entra como meio, não como contrato.
  // A lista de documentos da régua NÃO se escreve aqui — quem a produz é o doc_load.py,
  // e enumerá-la em prosa é o drift que o contrato existe pra matar.
  // Fail-open: régua ausente = eixo não roda, e isso NÃO é gap.
...
| `saudePrompt` | `MECANICO` | `destravadorPrompt` | `DESTRAVADOR` |
...
- `tickPlanPrompt` — **com schema `TICK_RESULT`**, e a lista inteira da onda num agente só. Papel **mecânico e só**: gravar no plano os passos que acabaram de sair, rodando **um comando por passo, em sequência**:
  ...
  cd <raiz> && python3 "$MARCADOR" --dir <raiz>/.claude/plans tick <plano> <taskId> --evidencia "<evidencia>"
  ```

  ⚠️ **O `<plano>` vai NO COMANDO, nunca numa observação ao lado.** `tick` recebe o id do plano como primeiro posicional, e sem ele o programa recusa assim que a pasta tem 2+ planos `active` (`há N planos ativos (…) — diga qual`). Em 2026-08-14 isso matou **três corridas seguidas** pela mesma porta: a marcação falhava, o bloco não fechava e a rodada seguinte lia `todo` de novo — 12 passos tiveram de ser marcados à mão depois. A receita correta chegou a ser escrita no `planText` em duas corridas e **nunca alcançou o marcador**: o texto da missão vai ao orquestrador e aos revisores, jamais aos papéis mecânicos. Por isso o id nasce no próprio comando, derivado do `planPath`.
