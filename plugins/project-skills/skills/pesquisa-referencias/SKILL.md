...
description: "Pesquisa de referências para a concepção — muitos agentes lendo projetos abertos e produtos pagos em paralelo para levantar repertório sobre um assunto que travou uma etapa do /start. O custo é declarado ANTES de qualquer leitura (quantos agentes, quanto tempo, quantas fontes) e a pesquisa só começa com o aceite do dono. Roda com travas de parada duras (teto de agentes, de fontes, de tempo e parada por saturação) e entrega um dossiê de achados com a fonte de cada um. Nada do que ela acha entra em documento autoral sozinho — achado é insumo de pergunta, nunca resposta. Quinta skill do plugin project-doc, ao lado do /doc, /doc-touch, /start e /design-md. Use quando o usuário diz \"/pesquisa-referencias\", \"pesquisa referências\", \"como os outros resolvem isso\", \"o que o mercado faz aqui\", \"me traz repertório\", ou quando uma etapa do /start empaca porque falta repertório para responder — e aí OFEREÇA, nunca dispare sozinho."
---
...
A entrevista do `/start` para quando o dono não tem repertório para responder. Não é falta de
...
- **A pesquisa não escreve documento autoral.** Ela escreve o dossiê e para. Quem escreve
  `.claude/docs/*.md` é o `/start`, com a fala do dono.
- **Documento com `status: approved` a pesquisa não toca**, nem para "complementar". Acordo fechado
  se reabre pela porta da frente — `/start review`, nova apresentação, nova aprovação.
...
- **Nunca toque em documento aprovado.** Acordo fechado se reabre pelo `/start review`.
