...
|---|---|---|---|
| 1 | **Autoral** | `quality-goals.md` · `constraints.md` · `context.md` · `solution-strategy.md` · `glossary.md` · `constituicao.md` | sempre |
...
**Onde cada documento mora, e quem tem direito de escrevê-lo** — pastas, as três naturezas
(autoral, minerado, derivado), o frontmatter e a tabela de quem escreve e quem lê — está em
`contrato-familia.md`, ao lado deste arquivo (fonte: `_shared/contrato-familia.md`). Ele vale para
a família inteira de skills de documentação; o `authorial-kit.md` diz o que escrever DENTRO de cada
documento autoral.
...
restrições → contexto → estratégia → glossário → **lei**. A ordem não é estética: as metas são o
critério que os outros consomem, e a lei é a última porque ela transforma o que os cinco anteriores
acordaram em artigo cobrável — escrevê-la antes seria adivinhar a prioridade que a etapa ainda não
fechou. Entre as etapas, a ordem é a da tabela acima — e a próxima só abre com a
anterior aprovada.
...
2. **Sabatine** com `/grill-me` — ou `/grill-me com-docs` quando já houver `CONTEXT.md` ou ADR para
...
**Se NÃO existe `CLAUDE.md`** (o cenário-bandeira desta skill — projeto virgem): escreva o **índice
mínimo** na raiz e os **cinco ponteiros finos** de agente, pelo molde `12 · Índice mínimo e
ponteiros` do `references/authorial-kit.md`. Sem isso o projeto sai da concepção desligado — os
documentos existem e nenhum agente sabe que existem.

O índice é **provisório e derivado**: só entra documento com `status: approved`, e cada linha sai do
...
- **`/principles` não é juiz da lei.** Ele audita contra princípios de sistema GENÉRICOS, que não
  conhecem este projeto; a `constituicao.md` é que manda no que ESTE sistema tem que ser. Em
  conflito, a constituição ganha, e princípio genérico só vira artigo se o dono o enunciar.
- **A sabatina não é juíza.** `/grill-me` e `/grill-me com-docs` são o caminho até o acordo, não o
