...
|---|---|---|---|
| 1 | **Acordo autoral** | `quality-goals.md` · `constraints.md` · `context.md` · `solution-strategy.md` · `glossary.md` · `constituicao.md` | sempre — são os 6 universais |
...
1. **Apresentar o documento inteiro** ao dono — o texto real, não um resumo dele. Ele não adivinha o
   que você escreveu.
2. **Sabatinar** com `/grill-me` (ou `/grill-me com-docs`, quando já existe `CONTEXT.md` ou ADR para
...
o `references/spec.md` da skill `design-md` (plugin `project-doc`) e valida com o CLI oficial:
...
- **Caminho de saída:** `.claude/docs/constituicao.md`. É este caminho, e só ele, que os motores
  abrem — o revisor de construção do `/sprint`, o `/qa-loop`, a auditoria e o gate de plano. Nome
  ou pasta diferente = lei que ninguém lê.
- **Pergunta que responde:** em que dimensões toda peça deste sistema é julgada, e **quem cobra**
  cada uma?
- **Conteúdo mínimo:** de 3 a 10 artigos. Cada artigo tem um nome, uma frase do que exige, e a
  linha **Quem cobra** — o comando, o teste ou o gate que reprova quem desobedece. Artigo sem
  cobrador continua no documento, marcado como dívida declarada; o que não pode é ele passar por
  cumprido.
- **Por que é da etapa 1, e por último dentro dela:** a lei é escrita **em cima** das metas, das
  restrições e da estratégia já acordadas — ela não inventa critério novo, ela transforma o que já
  foi acordado em artigo cobrável. Escrevê-la antes seria adivinhar a prioridade que a etapa ainda
  não fechou.
- **Critério de pronto:** dado um pedaço de código qualquer do projeto, dá para dizer qual artigo
  ele viola e qual comando prova a violação.

**Fronteira com `/principles`.** Aquela skill audita contra princípios de sistema **genéricos**, que
valem em qualquer projeto e são conselho; este documento diz o que **este** sistema tem que ser, e é
lei. Em conflito, a lei ganha. Princípio genérico não vira artigo sem o dono enunciá-lo.

**Não invente artigo.** Este documento tem a mesma trava dos outros cinco: o que não veio do dono
não entra. O que a mineração pode fazer é **oferecer o cobrador** — o teste, o script, o hook que
já existe no repositório — para o artigo que o dono acabou de enunciar.

**Roteiro:**
1. "Quando uma peça deste sistema é reprovada, o que ela costuma ter feito de errado?" (repita até
   sair de 3 a 10 dimensões)
2. Para cada uma: "escreva a exigência como uma ordem — o que toda peça TEM que fazer?"
3. "Hoje, se alguém desobedecer isso, o que reprova? Um teste, um gate, uma revisão humana, ou
   ninguém?"
4. Onde a resposta for "ninguém": "isso fica declarado como dívida, ou você quer que a gente crie o
   cobrador agora?"
5. "Se dois artigos se contradisserem num caso real, qual ganha?" (a ordem vira a ordem dos artigos)
...
> As dimensões em que toda peça deste sistema é julgada. Em conflito com qualquer outro
> documento, este ganha. Artigo sem cobrador é dívida declarada, não artigo cumprido.

## Artigo 1 · {nome da dimensão}

{o que exige, em uma frase de ordem}

- **Quem cobra:** {comando · teste · gate · "ninguém ainda — dívida declarada"}
- **Como está hoje:** {a prova do estado atual: a saída do comando, o número, o arquivo}
...
## Documentation Index
- **[{arquivo}.md](.claude/docs/{arquivo}.md)** — {a linha que abre o documento, literal} → leia quando {gatilho}
{uma linha por documento APROVADO, na ordem das etapas}
<!-- start-doc:index:end -->
```

Regras do molde, todas checáveis no arquivo escrito:

- **Só entra documento com `status: approved`.** `draft` e `ready` ficam de fora e viram linha de
  cobrança no relatório — índice não é lugar de anunciar texto que ninguém aprovou.
- **Cada linha é derivada do documento**, não da sua memória da conversa: o título e o resumo saem do
  corpo aprovado.
