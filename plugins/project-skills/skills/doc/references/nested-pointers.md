...
```markdown
<!-- nested-pointer gen=3.8 derived-from=.claude/docs/apps/{app}.md sig=<sig> — nao editar a mao -->
# {app-name}
...
## Router de módulo (organismo) ≠ nested-pointer (`apps/`)

Não confundir os dois artefatos derivados:

| | **nested-pointer** (`--nested`, este doc) | **module-router** (organismo, Caminho C) |
|---|---|---|
| onde | `apps/{app}/CLAUDE.md` | `{módulo}/.claude/CLAUDE.md` |
| marker | `<!-- nested-pointer … -->` | `<!-- project-doc:module-router … -->` |
| conteúdo | header-only (nome+porta+link) | preâmbulo do organismo + **gotchas de sobrevivência** inlined + link |
| por quê | edge-case do lazy-load de `apps/` | sobreviver a sparse-checkout (VPS sem o `.claude/` da raiz) |
| doc canônica | `.claude/docs/apps/{app}.md` | `.claude/docs/modules/{módulo}/*.md` (granular) |

O router NÃO é header-only de propósito: a doc canônica pode não estar materializada na VPS de
sparse-checkout onde se debugam incidentes, então o router carrega o mínimo de sobrevivência.
O template completo vive em `templates.md` → **Module Router Template**; as regras de geração/
census/arquivamento em `organism.md` → **Conformação de organismo (Caminho C)**.
