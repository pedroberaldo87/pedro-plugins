...
Roda com:  python3 plugins/project-skills/lib/test_journal.py
"""
import json
import os
import shutil
...
def test_engine_robust():
    ...
    import tempfile
    import collect_engine  # cópia vendorada em lib/ (HERE já está no sys.path)
    d = tempfile.mkdtemp()
...
def test_self_path_match():
    ...
    check("#10 sufixo de path casa",
          journal.self_path_match("lib/journal.py", {"plugins/project-skills/lib/journal.py"}) is True)
...
def test_fuse():
    ...
    def seed(base, events):
        d = os.path.join(base, ".claude", ".project-doc")
        os.makedirs(d, exist_ok=True)
        with open(os.path.join(d, "findings.jsonl"), "w", encoding="utf-8") as fh:
            for e in events:
                fh.write(json.dumps(e) + "\n")

    seed(T, [{"ev": "discovered", "id": "root1", "raw_kind": "commit", "text": "raiz", "anchors": [], "source": {}}])
    seed(os.path.join(T, "finance"), [
        {"ev": "discovered", "id": "mod1", "raw_kind": "handoff", "text": "gotcha do finance", "anchors": ["finance/x.ts"], "source": {}},
        {"ev": "discovered", "id": "mod2", "raw_kind": "handoff", "text": "outra do finance", "anchors": [], "source": {}},
        {"ev": "discovered", "id": "root1", "raw_kind": "commit", "text": "raiz", "anchors": [], "source": {}},  # DUP da raiz
    ])
    # mcp sem journal → deve ser pulado sem erro

    r = journal.run_fuse(T, ["finance", "mcp"])
    check("fuse absorve só os 2 novos (root1 é dup → ignorado)", r["fused"] == 2)
    check("fuse reporta finance como fonte", r["from"] == ["finance"])
    body = open(os.path.join(T, ".claude", ".project-doc", "findings.jsonl"), encoding="utf-8").read()
    check("fuse marca proveniência fused_from", '"fused_from": "finance"' in body)
    st = journal.fold(journal.read_events(T))
    ids = {f["id"] for f in journal.live_findings(st)}
    check("fuse: mod1+mod2 vivos na raiz", "mod1" in ids and "mod2" in ids)

    # idempotência: re-rodar não re-appenda
    r2 = journal.run_fuse(T, ["finance", "mcp"])
    check("fuse idempotente (re-rodar → fused=0)", r2["fused"] == 0)
    body2 = open(os.path.join(T, ".claude", ".project-doc", "findings.jsonl"), encoding="utf-8").read()
    check("fuse idempotente não duplica mod1", body2.count('"id": "mod1"') == 1)
    shutil.rmtree(T, ignore_errors=True)
