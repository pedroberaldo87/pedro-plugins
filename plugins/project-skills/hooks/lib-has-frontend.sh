...
has_frontend() {
  ...
  git -C "$dir" ls-files 2>/dev/null | grep -qE '\.(tsx|jsx|vue|svelte)$' && return 0
  git -C "$dir" ls-files 2>/dev/null | grep -qE '(^|/)index\.html$' && return 0
  git -C "$dir" ls-files 2>/dev/null | grep -qE '\.(swift|kt)$' && return 0
