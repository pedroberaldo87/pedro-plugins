...
# 1) projeto sem doc -> DENY mandando /start
OUT=$(gate EnterPlanMode "$BARE"); D=$(decision "$OUT")
[ "$D" = "deny" ] && ok "sem doc: EnterPlanMode negado" || bad "sem doc: EnterPlanMode negado" deny "$D"
case "$(reason "$OUT")" in *"/start"*) ok "sem doc: a mensagem manda rodar /start" ;;
  *) bad "sem doc: mensagem cita /start" "cita /start" "$(reason "$OUT" | head -c 60)" ;; esac
...
# A9) OS 8 DOCUMENTOS FECHADOS COMO O /start MANDA (status: approved + approved:)
...
# como o /start grava. A marca é calculada antes e escrita depois: se ela fosse
...
# A rodada é a skill, e o nome MORTO dela (`/project-doc`, da casa extinta em
# 2026-08-09) é pedido de skill inexistente — o vivo é `/project-skills:doc`.
printf '%s' "$M" | grep -qE '/project-doc|/start-doc' && bad "não escreve o nome morto" "sem /project-doc nem /start-doc" "$M" \
