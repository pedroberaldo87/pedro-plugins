...
**Status:** rascunho para validação · **Autor:** mantenedor do plugin · **Data:** 2026-06-19
...
### 1.3 Escala real (monorepo de referência)
...
- **Secundário: o humano dono do projeto**, que raramente lê a doc direto, mas precisa que o agente opere (deploy, SSH) sem ele relembrar credenciais.
...
- `/doc` — **delta** (forward+backward) + projeta. Padrão.
- `/doc --deep` — minera **todas** as sessões cruas (cold-start / backfill do tier 4).
- `/doc --rebuild` — descarta a doc e re-projeta do **journal inteiro** (idempotente).
...
- **Localização:** `plugins/project-skills/hooks/hooks.json` (subpasta `hooks/` — gotcha conhecido do projeto: na raiz o Claude ignora silenciosamente).
...
- **Cold-start das 303 sessões do tools** — a estratégia de atacar o primeiro mergulho dos 270 MB fica para quando o usuário rodar.
- **Trigger automático** — o project-doc continua rodando sob comando; integração com o fim de sessão / `/handoff` fica para depois.
- **Deploy** — a implementação para antes de subir; `sync-shared.sh`, commit, `plugin tag` e atualização dos plugins ficam com o mantenedor.
...
|---|---|---|
| `plugins/doc` | 2.2.1 → **3.0.0** | mudança maior (nova arquitetura) |
