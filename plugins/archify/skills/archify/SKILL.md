...
1a. **For `workflow`, start from the template**, not from a blank file: `templates/fluxo.template.json` carries the whole vocabulary in one flow — actor lane, steps, a decision with named exits, two parallel branches, the exception lane with its retry, recorded states, phases, groups and `mainPath` — with the rendered proof beside it in `templates/fluxo.template.html`, whose third card ("Recursos extras") lists the optional features you can switch on (`views`, `phases`/`groups`, `mainPath`, `animation: trace`). Copy it, rename the ids, replace the labels; keep the roles. This template is the single source: other skills point at it instead of restating the vocabulary.
1b. **For `architecture`, start from the template**, not from a blank file: `templates/arquitetura.template.json` carries the whole vocabulary in one diagram — layers, zone, datastores (volatile and durable), queue, trust boundary, externals, identity guard, and protocols on the edge labels — with the rendered proof beside it in `templates/arquitetura.template.html`. Copy it, rename the ids, replace the labels; keep the roles. This template is the single source: other skills point at it instead of restating the vocabulary.
1c. **A case the base vocabulary has no word for goes in DECLARED, never drawn ad-hoc**: keep one of the seven base `type`s and name the extra role in `tag` (a clock-driven scheduler is `{"type": "backend", "tag": "cron"}`; a wait on a human is `{"type": "security", "tag": "espera humana"}`). Both templates carry this rule in their last card, "Extensão declarada", with the worked case. A role `tag` cannot carry is a request for a new type in archify — never hand-placed SVG.
