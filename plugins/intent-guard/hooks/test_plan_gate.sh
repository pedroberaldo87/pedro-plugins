...
MOCKD="$(mktemp -d "$(td_tmpdir)"/ig-pg-mock-XXXXXX)"
trap 'rm -rf "$REPO" "$MOCKD" "$TMPD"/intent-guard-plandeny-pgsid' EXIT
...
export INTENT_GUARD_JUDGE_CMD="$MOCKD/mock_judge_miss.sh"
cat > "$MOCKD/mock_judge_miss.sh" <<'EOF'
#!/usr/bin/env bash
cat >/dev/null
echo '{"classify":[{"ev":"classify","raw":"r-1","class":"pedido","resumo":"export CSV ;","substitui":null}],"missing":[{"id":"p-1","resumo":"export CSV ;"}]}'
EOF
chmod +x "$MOCKD/mock_judge_miss.sh"
...
export INTENT_GUARD_JUDGE_CMD="$MOCKD/mock_judge_ok.sh"
cat > "$MOCKD/mock_judge_ok.sh" <<'EOF'
#!/usr/bin/env bash
cat >/dev/null; echo '{"classify":[],"missing":[]}'
EOF
chmod +x "$MOCKD/mock_judge_ok.sh"
...
export INTENT_GUARD_JUDGE_CMD="$MOCKD/mock_judge_miss2.sh"
cat > "$MOCKD/mock_judge_miss2.sh" <<'EOF'
#!/usr/bin/env bash
cat >/dev/null; echo '{"classify":[],"missing":[{"id":"p-1","resumo":"x"}]}'
EOF
chmod +x "$MOCKD/mock_judge_miss2.sh"
...
export INTENT_GUARD_JUDGE_CMD="$MOCKD/mock_judge_miss3.sh"
cat > "$MOCKD/mock_judge_miss3.sh" <<'EOF'
#!/usr/bin/env bash
cat >/dev/null; echo '{"classify":[],"missing":[{"id":"p-1","resumo":"x"}]}'
EOF
chmod +x "$MOCKD/mock_judge_miss3.sh"
