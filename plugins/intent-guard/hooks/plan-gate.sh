...
N_PEND="$(printf '%s' "$STATE" | "$JQ" '.pending | length')"
N_LIVE="$(printf '%s' "$STATE" | "$JQ" '.live | length')"
...
printf '%s' "$PARSED" | "$JQ" -c '.classify[]?' | "$PY" "$LEDGER" apply --cwd "$CWD" 2>/dev/null

MISSING="$(printf '%s' "$PARSED" | "$JQ" -r '[.missing[]? | "\(.id): \(.resumo)"] | join("; ")')"
[ -z "$MISSING" ] && exit 0

# --- teto anti-loop: máx 2 devoluções por sessão ---
DENYF="/tmp/intent-guard-plandeny-${SID}"
